...
public:
    ...
    std::string phone;
    std::string username;
    std::string password; // Input-only. Repository never loads password values from SQL.
    double gpa;
    double tuitionOwed;
    std::string status;
...
public:
    ...
    bool isTuitionOwnedByStudent(const std::string& studentId, const std::string& tuitionId);
